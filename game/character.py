...
def enforce_rules(result: dict, player_input: str, ap: int) -> dict:
    ...
    if any(kw in player_input for kw in RUDE_KEYWORDS):
        ...
        return result  # ← return เลย ไม่ต้อง override ซ้ำ

    # ✅ แค่ clamp invalid mood เท่านั้น อย่า override ที่ LLM ตั้งมาแล้วถูก
    if result['mood'] not in VALID_MOODS:
        result['mood'] = 'neutral'

    # AP ต่ำห้ามแสดง happy/touched
...
# ── Main Function ──────────────────────────────────────────────────────
def call_fern(player_input: str, ep: dict, ap: int, tp: int) -> dict:
    for attempt in range(2):
        try:
            response = fern_chain.invoke({
                'setting'     : ep['setting'],
                'context'     : ep['context'],
                'ap'          : ap,
                'tp'          : tp,
